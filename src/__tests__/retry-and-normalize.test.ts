...
describe("retryTurn missing source_images guard", () => {
  function canRetry(turn: { mode: string; source_images?: File[] }): { canRetry: boolean; reason?: string } {
    if ((turn.mode === "edit" || turn.mode === "upscale") && !turn.source_images?.length) {
  ...
  it("should block retry for upscale turn without source_images", () => {
    const turn = { mode: "upscale" as const, source_images: undefined };
    const result = canRetry(turn);
    expect(result.canRetry).toBe(false);
  });
  ...
  it("should allow retry for upscale turn with source_images present", () => {
    const turn = { mode: "upscale" as const, source_images: [new File([], "test.png")] };
    const result = canRetry(turn);
    expect(result.canRetry).toBe(true);
  });
});
