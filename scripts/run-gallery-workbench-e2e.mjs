...
async function clickFirstVisibleAction(page, ariaLabel) {
  const firstCard = page.locator(".example-gallery-grid .examples-card").first();
  ...
  const actions = page.locator(`button[aria-label='${ariaLabel}']`);
...
async function main() {
  ...
  try {
    ...
    await runStep("推荐/专题切换 + 专题展开 + 来源移除", async () => {
      ...
      await page.getByText("UI / 社媒截图与产品展示").waitFor();

      const topicButton = page.locator('button[aria-label^="打开专题："]').first();
